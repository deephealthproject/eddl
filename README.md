...
mkdir build
cd build
cmake .. -DBUILD_TARGET=CUDNN -DBUILD_HPC=OFF -DBUILD_SUPERBUILD=ON -DBUILD_EXAMPLES=OFF -DProtobuf_INCLUDE_DIRS=${PROTOBUF_INCLUDE_DIRS} -DEIGEN3_INCLUDE_DIR=${EIGEN_INCLUDE_DIRS}
make -j$(nproc)
sudo make install
```
