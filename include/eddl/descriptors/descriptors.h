...
class ConvolDescriptorT3D {
public:
    vector<int> ksize;
    vector<int> stride;
    vector<int> pads; // {depth-front, depth-back, rows-top, rows-bottom, cols-left, cols-right}
    ...
    ConvolDescriptorT3D(int filters, const vector<int> &kernel_size, const vector<int> &strides, string padding, const vector<int> &pads,
                        int groups, const vector<int> &dilation_rate, bool use_bias, int mem=0);
