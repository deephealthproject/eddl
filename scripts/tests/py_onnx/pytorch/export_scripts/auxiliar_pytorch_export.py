...
class Net(nn.Module):
    def __init__(self):
        ...
        self.split1_block1 = nn.Sequential(
            nn.ZeroPad2d(1),
            nn.Conv2d(10, 16, 3, 1, 0),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.AvgPool2d(2, 2))
        self.split1_block2 = nn.Sequential(
            nn.ZeroPad2d(2),
            nn.Conv2d(10, 16, 5, 1, 0),
            nn.BatchNorm2d(16),
            nn.ReLU(),
            nn.AvgPool2d(2, 2))
        self.split1_block3 = nn.Sequential(
        ...
        self.split2_block1 = nn.Sequential(
            nn.ZeroPad2d(1),
            nn.Conv2d(44, 16, 3, 1, 0),
            nn.BatchNorm2d(16),
            nn.ReLU())
        self.split2_block2 = nn.Sequential(
            nn.ZeroPad2d(2),
            nn.Conv2d(20, 16, 5, 1, 0),
            nn.BatchNorm2d(16),
            nn.ReLU())
    ...
    def forward(self, x):
        x = self.block1(x)
        x1, x2, x3 = torch.split(x, 10, 1)
        x1 = self.split1_block1(x1)
        x2 = self.split1_block2(x2)
        x3 = self.split1_block3(x3)
        x = torch.cat([x1, x2, x3], 1)
        x = self.block2(x)
        x1, x2 = torch.split(x, (44, 20), 1)
        x1 = self.split2_block1(x1)
        x2 = self.split2_block2(x2)
        x = torch.cat([x1, x2], 1)
        out = self.block3(x)
        return out
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='PyTorch Conv MNIST example with auxiliar layers')
