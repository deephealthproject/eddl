...
from tensorflow.keras.layers import Dense, Input, LeakyReLU, SimpleRNN
...
model = Sequential()
model.add(Input(shape=(28, 28), name="linput"))
model.add(Dense(32))
model.add(LeakyReLU())
...
# Convert to ONNX
onnx_model = keras2onnx.convert_keras(model, "rnn_mnist", debug_mode=1)
# Save ONNX to file
