...
parser = argparse.ArgumentParser(
    description='Keras Dilated Conv1D MNIST Example')
...
parser.add_argument('--output-path', type=str, default="onnx_models/dilated_conv1D_mnist.onnx",
...
# Make sure images have shape (784, 1)
...
model.add(Conv1D(16, 5, padding="same", dilation_rate=2, activation="relu"))
model.add(MaxPooling1D(2, 2))
model.add(Conv1D(16, 3, padding="valid", dilation_rate=4, activation="relu"))
model.add(Conv1D(16, 2, padding="valid", dilation_rate=3, activation="relu"))
model.add(MaxPooling1D(2, 2))
model.add(Flatten())
...
# Convert to ONNX
input_spec = (tf.TensorSpec((args.batch_size, 784, 1), dtype=tf.float32),)
