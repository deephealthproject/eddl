...
#define BLOCK_SIZE 8              // block size is the number of pixels to read/write per cycle
struct block_t {
  ...
  if (enable) {
    ...
    for (int r=0; r<H*W/BLOCK_SIZE; r++) {
      ...
      ptr[r+offset] = data;
		...
		printf("[4] = %6.2f  ",data.pixel[4]);
		printf("[5] = %6.2f  ",data.pixel[5]);
		printf("[6] = %6.2f  ",data.pixel[6]);
		printf("[7] = %6.2f  ",data.pixel[7]);
		printf("\n"); 
    }
  } else {
	  printf("NO WRITE  %d\n", offset);
...
static void sigmoid(hls::stream<pixel_out_t> &in, hls::stream<pixel_out_t> &out, int H, int W, int O_ITER, int enable) {
	  ...
	  if(enable){
		  ...
		  for(int i = 0; i < H*W*O_ITER; i++){	
				...
				out_p.pixel[4] =  1/(1+exp(-data.pixel[4]));
				out_p.pixel[5] =  1/(1+exp(-data.pixel[5]));
				out_p.pixel[6] =  1/(1+exp(-data.pixel[6]));
				out_p.pixel[7] =  1/(1+exp(-data.pixel[7]));
				
				printf("[0] = %6.2f  ",out_p.pixel[0]);
				printf("[1] = %6.2f  ",out_p.pixel[1]);
				printf("[2] = %6.2f  ",out_p.pixel[2]);
				printf("[3] = %6.2f  ",out_p.pixel[3]);
				printf("[4] = %6.2f  ",out_p.pixel[4]);
				printf("[5] = %6.2f  ",out_p.pixel[5]);
				printf("[6] = %6.2f  ",out_p.pixel[6]);
				printf("[7] = %6.2f  ",out_p.pixel[7]);
				printf("\n"); 
				out << out_p;
...
static void cvt_maxpool(int WI, int HI, hls::stream<pixel_out_t> &in, hls::stream<frame_m> &out) {
    ...
    for (int hi=0; hi < HI; hi++) {
        ...
        for (int wi=0; wi < WI; wi++) {
			...
			printf("[4] = %6.2f  ",pixel.pixel[4]);
			printf("[5] = %6.2f  ",pixel.pixel[5]);
			printf("[6] = %6.2f  ",pixel.pixel[6]);
			printf("[7] = %6.2f  ",pixel.pixel[7]);
			printf("\n"); 
...
static void mpool2d(int WO, int HO, hls::stream<frame_m> &in, hls::stream<pixel_out_t> &out) {
    ...
    for (int i=0; i < size_out; i++) {
		...
		printf("[4] = %6.2f  ",pix.pixel[4]);
		printf("[5] = %6.2f  ",pix.pixel[5]);
		printf("[6] = %6.2f  ",pix.pixel[6]);
		printf("[7] = %6.2f  ",pix.pixel[7]);
		printf("\n"); 
    }
}

static void maxpool(int W, int H, int WO, int HO, hls::stream<pixel_out_t> &in, hls::stream<pixel_out_t> &mpool_out, hls::stream<frame_m> &cvt_out_maxpool, int enable, int O_ITER){
...
void k_conv2D_8x8_resize
  ...
  #pragma HLS INTERFACE m_axi port=ptr_data0 offset=slave bundle=gmem  
  ...
  #pragma HLS INTERFACE m_axi port=ptr_data4 offset=slave bundle=gmem4  
  #pragma HLS INTERFACE m_axi port=ptr_data5 offset=slave bundle=gmem5  
  #pragma HLS INTERFACE m_axi port=ptr_data6 offset=slave bundle=gmem6  
  #pragma HLS INTERFACE m_axi port=ptr_data7 offset=slave bundle=gmem7  
  #pragma HLS INTERFACE m_axi port=ptr_kernel offset=slave bundle=gmem8 
  #pragma HLS INTERFACE m_axi port=ptr_bias offset=slave bundle=gmem9   
  #pragma HLS INTERFACE m_axi port=ptr_out0  offset=slave bundle=gmem   
  ...
  #pragma HLS INTERFACE m_axi port=ptr_out4  offset=slave bundle=gmem4   
  #pragma HLS INTERFACE m_axi port=ptr_out5  offset=slave bundle=gmem5  
  #pragma HLS INTERFACE m_axi port=ptr_out6  offset=slave bundle=gmem6   
  #pragma HLS INTERFACE m_axi port=ptr_out7  offset=slave bundle=gmem7 
  ...
  for (int o_iter = 0; o_iter<O_ITER; o_iter++) {
    ...
    int offset_read_data_channel_4 = channel_offset_reading * 4;
    int offset_read_data_channel_5 = channel_offset_reading * 5;
    int offset_read_data_channel_6 = channel_offset_reading * 6;
    int offset_read_data_channel_7 = channel_offset_reading * 7;

    // channel offsets for writing
	int channel_offset = (WO * HO) / BLOCK_SIZE; //if wo * ho < block size, no escribe
    int offset_write_data_channel_0 = ((o_iter << 3) * channel_offset);
    int offset_write_data_channel_1 = channel_offset + ((o_iter << 3) * channel_offset);
    int offset_write_data_channel_2 = (channel_offset * 2) + ((o_iter << 3) * channel_offset);
    int offset_write_data_channel_3 = (channel_offset * 3) + ((o_iter << 3) * channel_offset);
    int offset_write_data_channel_4 = (channel_offset * 4) + ((o_iter << 3) * channel_offset);
    int offset_write_data_channel_5 = (channel_offset * 5) + ((o_iter << 3) * channel_offset);
    int offset_write_data_channel_6 = (channel_offset * 6) + ((o_iter << 3) * channel_offset);
    ...
    read_data_channel(HIN, WIN, I_ITER, ptr_data4, offset_read_data_channel_4, out_read_channel_4, enable_read4, 4);
    read_data_channel(HIN, WIN, I_ITER, ptr_data5, offset_read_data_channel_5, out_read_channel_5, enable_read5, 5);
    read_data_channel(HIN, WIN, I_ITER, ptr_data6, offset_read_data_channel_6, out_read_channel_6, enable_read6, 6);
    read_data_channel(HIN, WIN, I_ITER, ptr_data7, offset_read_data_channel_7, out_read_channel_7, enable_read7, 7);
    join(HIN, WIN, I_ITER, out_read_channel_0, out_read_channel_1, out_read_channel_2, out_read_channel_3, out_read_channel_4, out_read_channel_5, out_read_channel_6, out_read_channel_7, out_read_data);
	...
	maxpool(W, H, WO, HO, out_relu, mpool_out, cvt_out_maxpool, enable_maxpool, O_ITER);
	sigmoid(mpool_out, out_sigmoid, HO, WO, O_ITER, enable_sigmoid);
    split(HO, WO, out_sigmoid, out_write_channel_0, out_write_channel_1, out_write_channel_2, out_write_channel_3, out_write_channel_4, out_write_channel_5, out_write_channel_6, out_write_channel_7);
    ...
    write_data_channel(HO, WO, ptr_out4, offset_write_data_channel_4, out_write_channel_4, enable_write4, 4);
    write_data_channel(HO, WO, ptr_out5, offset_write_data_channel_5, out_write_channel_5, enable_write5, 5);
    write_data_channel(HO, WO, ptr_out6, offset_write_data_channel_6, out_write_channel_6, enable_write6, 6);
    write_data_channel(HO, WO, ptr_out7, offset_write_data_channel_7, out_write_channel_7, enable_write7, 7);
