...
//    setDims(A);
//
//    repeat_nn<<<dimGrid,dimBlock>>>(A->ptr, A->shape[2], A->shape[3], B->ptr, B->shape[2], B->shape[3], r,c, size.data(), size.size());
...
//    setDims(A);
//
//    d_repeat_nn<<<dimGrid,dimBlock>>>(D->ptr, D->shape[2], D->shape[3], A->ptr, A->shape[2], A->shape[3], r, c, size.data(), size.size());
