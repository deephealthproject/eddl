...
__global__ void gpu_pad(float* A, float* B, int batch, int depth, int irows, int icols, int orows, int ocols, int padtb, int padlr){
    long int thread_id_x = threadIdx.x+blockIdx.x*blockDim.x;
    long int ops = batch*depth*irows*icols;
    ...
    if (thread_id_x < ops){
        ...
        int B_pos = b*B_stride[0] + c*B_stride[1] + (Ai+padtb)*B_stride[2] + (Aj+padlr)*B_stride[3];
        B[B_pos] = A[thread_id_x];
    }
}

__global__ void gpu_pad_back(float* A, float* B, int batch, int depth, int irows, int icols, int orows, int ocols,  int padtb, int padlr){
    long int thread_id_x = threadIdx.x+blockIdx.x*blockDim.x;
    long int ops = batch * depth*irows*icols;
