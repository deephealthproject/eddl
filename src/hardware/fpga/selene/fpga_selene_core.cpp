...
void fpga_device_init(int platform_type) {
  #ifdef FPGA_DEBUG
  printf("initializing SELENE SoC\n");
  #endif
  ...
  OCL_CHECK(err, context = new cl::Context(NULL, NULL, NULL, NULL, &err));
  OCL_CHECK(err, q = new cl::CommandQueue(*context, NULL, /*CL_QUEUE_PROFILING_ENABLE | CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE*/0, &err));
    
  std::string device_name = "SELENE-SoC"; //device.getInfo<CL_DEVICE_NAME>();
  ...
  OCL_CHECK(err, program = new cl::Program(*context, NULL, NULL, NULL, &err));
  ...
  for (int k=0; k<hlsinf_num_kernels; k++) {
    ...
    OCL_CHECK(err, kernel_hlsinf[k] = cl::Kernel(*program, "conv", &err));
...
//  data_ddr.param = 0;
  ...
  OCL_CHECK(err, buffer = new cl::Buffer(*context, /*CL_MEM_READ_WRITE*/0 /*| CL_MEM_EXT_PTR_XILINX*/, size, NULL /*&data_ddr*/, &err));
  return (void *)buffer;
}
...
//  float *newdata = (float *)memalign(4096, size*sizeof(float));

  OCL_CHECK(err, err= (*q).enqueueWriteBuffer(*cast_fpga_ptr, CL_TRUE, 0, size, ptr_cpu/*newdata*/, NULL, NULL));
  //(*q).finish();
//  printf("NO CRASH writebuffer\n");
