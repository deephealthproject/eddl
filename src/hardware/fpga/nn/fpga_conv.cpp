...
// from the CPU
//
void fpga_conv2D_8x8(cl::Buffer I, int Irows, int Icols, int Ichannels, cl::Buffer K, cl::Buffer B, cl::Buffer O, int Ochannels) {
  ...
  int O_ITER 0 (Ochannels + (CPO-1)) / CPO;
  ...
  OCL_CHECK(err, err = kernel_conv2D_K3x3_S1x1_P1x1_BS1.setArg(arg++, I_ITER);
  OCL_CHECK(err, err = kernel_conv2D_K3x3_S1x1_P1x1_BS1.setArg(arg++, O_ITER);
  ...
  // Launch the Kernel
  OCL_CHECK(err, err = q.enqueueNDRangeKernel(kernel_conv2D_K3x3_S1x1_P1x1_BS1, 0, 1, 1, NULL, &kernel_events[0]));
...
void fpga_conv2D(ConvolDescriptor *D)
{
  ...
  if ((stride_rows == 1) && (stride_cols == 1) && (Krows == 3) && (Kcols == 3) && 
    ...
    if (!D->kernel_in_fpga_format) {
      fpga_reshape_kernel_data_convol(D, 3, 3, Ichannels, Ochannels, 8, 8);
      D->kernel_in_fpga_format = 1;
    }
    fpga_conv2D_8x8(I, Irows, Icols, Ichannels, K, B, O, Ochannels);
    return;
  }
  #endif
