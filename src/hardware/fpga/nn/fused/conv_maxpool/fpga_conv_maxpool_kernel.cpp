...
// -----------------------------------------------------------------
// Conv2D + Maxpool of kernels
//
//
//TODO
// fpga_conv_maxpool_kernel: This function launches the kernel execution on the FPGA
void fpga_conv_maxpool_kernel(cl::Buffer I, int Irows, int Icols, int Ichannels, 
...
void fpga_conv_maxpool_launch(cl::Buffer I, int Irows, int Icols, int Ichannels, 
  ...
  if (num_kernels == 1) {
    ...
    fpga_conv_maxpool_kernel(I, Irows, Icols, Ichannels, K, B, O, Ochannels, CPI, CPO, first_o_iter, last_o_iter, 0);
  } else {
    ...
    for (int k=0; k<num_kernels_to_launch; k++) {
      ...
      fpga_conv_maxpool_kernel(I, Irows, Icols, Ichannels, K, B, O, Ochannels, CPI, CPO, first_o_iter, last_o_iter,  k);
    }
  }
}
...
void fpga_conv_maxpool_transform(ConvolDescriptor *D) {
  ...
  if ((stride_rows == 1) && (stride_cols == 1) && (Krows == 3) && (Kcols == 3) && (batch_size == 1) && (padding_rows == 1) && (padding_cols == 1)) {
    ...
    int enable_maxp = 0;
    fpga_conv_maxpool_launch(I, Irows, Icols, Ichannels, K, B, O, Ochannels, k_conv2d_cpi, k_conv2d_cpo, k_conv2d_num_kernels, k_conv2d_max_rows);
