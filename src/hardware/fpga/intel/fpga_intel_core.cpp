...
//    PROFILING_HEADER(FPGA_WRITE);
//    OCL_CHECK(err, err= (*q).enqueueWriteBuffer(*cast_fpga_ptr, CL_TRUE, 0, size*sizeof(ap_int<8>), cpu_buff, nullptr, &blocking_event));
//    (*q).finish();
//    PROFILING_FOOTER(FPGA_WRITE);
//    free(cpu_buff);
...
//    PROFILING_FOOTER(Precision_Conversion);
//    PROFILING_HEADER(FPGA_WRITE);
//    OCL_CHECK(err, err= (*q).enqueueWriteBuffer(*cast_fpga_ptr, CL_TRUE, 0, size*sizeof(ap_int<32>), cpu_buff, nullptr, &blocking_event));
//    (*q).finish();
//    PROFILING_FOOTER(FPGA_WRITE);
//    free(cpu_buff);
//  } else {
 ...
 printf("  params: copy_cpu_to_fpga %d, copy_fpga_to_cpu %d, transform %d\n", copy_cpu_to_fpga, copy_fpga_to_cpu, transform);
  ...
  if (!transform && copy_cpu_to_fpga) {
    ...
    if (hlsinf_input_format == HLSINF_FP32) {
      ...
      PROFILING_FOOTER(Precision_Conversion);
      if (B->fpga_ptr == NULL) B->fpga_ptr = fpga_create_memory(FPGA_CLMEM_READ_ONLY, B->size * sizeof(ap_int<8>));
      ...
      PROFILING_FOOTER(Precision_Conversion);
      if (B->fpga_ptr == NULL) B->fpga_ptr = fpga_create_memory(FPGA_CLMEM_READ_ONLY, B->size*sizeof(ap_uint<8>));
