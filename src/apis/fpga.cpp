    ...
    if (LConv *conv = dynamic_cast<LConv*>(cl)) { 
      ...
      if (layer_src_dense != NULL) {
        ...
        if (hlsinf_filter_format == HLSINF_FP32) {
          ...
          layer_dst->filter->fpga_ptr = fpga_create_memory(FPGA_CLMEM_READ_ONLY, layer_dst->filter->size);  
        ...
        // dense bias
          ...
          if (hlsinf_bias_format == HLSINF_FP32) {
            ...
            layer_dst->bias->fpga_ptr = fpga_create_memory(FPGA_CLMEM_READ_ONLY, layer_dst->bias->size*4);  
