...
namespace eddl {
    ...
    void build(model net, optimizer o, CompServ *cs, bool init_weights) {
        ...
        net->build(o,{},
        {
        }, cs, init_weights, do_optimizer_delete, do_compserv_delete);
    }
    ...
    compserv CS_CPU(int th, string mem) {
        if (mem == "low_mem") return new CompServ(th,{},
            {
            }, 0, 2);
        else if (mem == "mid_mem") return new CompServ(th,{},
            {
            }, 0, 1);
        else if (mem == "full_mem") return new CompServ(th,{},
            {
            }, 0, 0);
    ...
    layer CenteredCrop(layer parent, vector<int> size, bool reshape, float constant, string name) {
        ...
        int y2 = y1 + size[0] - 1;
        int x2 = x1 + size[1] - 1;
        return new LCrop(parent,{y1, x1},
        {
            y2, x2
        }, reshape, constant, name, DEV_CPU, 0);
    }
    ...
    layer BatchNormalization(layer parent, float momentum, float epsilon, bool affine, string name) {
        ...
        if (parent->output->shape.size() == 3) {
            parent = _expand3d_to_4d(parent, "BatchNormalization");
        }
    ...
    layer GlobalMaxPool(layer parent, string name) {
        // Expand dimension if needed
        if (parent->output->shape.size() == 3) {
            parent = _expand3d_to_4d(parent, "GlobalMaxPool");
        }
    ...
    layer GlobalAveragePool(layer parent, string name) {
        // Expand dimension if needed
        if (parent->output->shape.size() == 3) {
            parent = _expand3d_to_4d(parent, "GlobalAveragePool");
        }
    ...
    layer GlobalAveragePool1D(layer parent, string name) {
        ...
        if (name.empty()) {
            name = "GlobalAveragePool1D";
        } // Set default name
        return AveragePool1D(parent,{h},
        {
            1
        }, "none", name);
    }
    ...
    layer GlobalAveragePool3D(layer parent, string name) {
        ...
        if (name.empty()) {
            name = "GlobalAveragePool3D";
        } // Set default name
        return AveragePool3D(parent,{d, h, w},
        {
            1, 1, 1
        }, "none", name);
    }
    ...
    Net* download_vgg16(bool top, vector<int> input_shape) {
        ...
        if (top) {
            ...
            Layer *l = getLayer(net, "flatten_60");
            l->name = "top";
    ...
    Net* download_resnet18(bool top, vector<int> input_shape) {
        ...
        if (top) {
            ...
            Layer *l = getLayer(net, "flatten_170");
            l->name = "top";
    ...
    Net* download_resnet34(bool top, vector<int> input_shape) {
        ...
        if (top) {
            ...
            Layer *l = getLayer(net, "flatten_306");
            l->name = "top";
    ...
    Net* download_resnet50(bool top, vector<int> input_shape) {
        ...
        if (top) {
            ...
            Layer *l = getLayer(net, "flatten_473");
            l->name = "top";
    ...
    Net* download_resnet101(bool top, vector<int> input_shape) {
        ...
        if (top) {
            ...
            Layer *l = getLayer(net, "flatten_932");
            l->name = "top";
    ...
    Net* download_resnet152(bool top, vector<int> input_shape) {
        ...
        if (top) {
            ...
            Layer *l = getLayer(net, "flatten_1391");
            l->name = "top";
