...
void get_fpga_model_params(Net * fpga_model) {
  ...
  model model_for_fpga(model m_src) {
      ...
      int found_Sp;       // current layer is a Sofplus layer
      int found_Tanh;     // current layer is a Tanh layer
	...
	//found_CL = found_C && found_nL && supported_stride && supported_kernel && supported_padding;
          ...
          if(found_CSTMA) {
            //if we have found a CSTMA layer we have to transform the all the ADD parents
            for (int x=0; x<nnnnl->parent.size(); x++) {
              if (nnnnl->parent[x] != nnnl) {
