    ...
    padcl = pad[2]; padcr = pad[3];  // cols: left-right
       ...
       if(!A->isCPU()){
           if(pad[0] != pad[1] || pad[2] != pad[3]){
              msg("Warning: asymmetric padding not supported by cuDNN... fixing ... potential shapes mismatch later", "pool2d");
...
int PoolDescriptor::compute_output(const string& padding, int input_size, int kernel_size, int stride, int dilation_rate){
...
int PoolDescriptor::compute_output(vector<int> padding, int input_size, int kernel_size, int stride, int dilation_rate) {
...
vector<int> PoolDescriptor::compute_padding(int output_size, int input_size, int kernel_size, int stride, string padding, bool row){
