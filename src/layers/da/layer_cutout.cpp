...
Layer *LCutout::share(int c, int bs, vector<Layer *> p) {
    auto *n = new LCutout(p[0], this->from_coords, this->to_coords, this->constant,  this->name, this->dev, this->mem_level);
    n->orig = this;
...
Layer *LCutout::clone(int c, int bs, vector<Layer *> p, int todev) {
    auto *n = new LCutout(p[0], this->from_coords, this->to_coords, this->constant,  name, todev, this->mem_level);
    n->orig = this;
