...
LConvMaxPool::LConvMaxPool(Layer *parent, int filters, const vector<int> &kernel_size,
        ...
        LConvMaxPool(parent, new ConvolDescriptor(filters, kernel_size, conv_strides, conv_padding, pads, groups, dilation_rate, use_bias, mem), new PoolDescriptor(pool_size, pool_strides, pool_padding, mem), name, dev, mem) {
};
...
LConvMaxPool::LConvMaxPool(Layer *parent, ConvolDescriptor *D,  PoolDescriptor *P, string name, int dev, int mem) : LinLayer(name, dev, mem) {
    if (parent->output->ndim != 4) msg("LConvMaxPool only works over 4D tensors", "LConvMaxPool::LConvMaxPool");

    // Set default name
    if(name.empty()) this->name = "conv2d_maxpool_" + to_string(++total_layers);
