...
void LHLSinf::forward() {
    ...
    if (filter->fpga_ptr == NULL) {
      if (hlsinf_filter_format == HLSINF_FP32) {
        ...
        fpga_copy_memory_to_fpga_and_format(filter->ptr, (cl_mem)filter->fpga_ptr, filter->size, HLSINF_FP32, HLSINF_API8);
      } else {
    ...
    if (bias->fpga_ptr == NULL) {
      if (hlsinf_bias_format == HLSINF_FP32) {
        ...
        fpga_copy_memory_to_fpga_and_format(bias->ptr, (cl_mem)bias->fpga_ptr, bias->size, HLSINF_FP32, HLSINF_API32);
      } else {
