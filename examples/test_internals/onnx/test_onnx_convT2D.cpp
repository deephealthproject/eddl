...
//////////////////////////////////
// test_onnx_convT2D.cpp:
// A MNIST encoder-decoder example
// with convT2D net to test the
// ONNX module
//////////////////////////////////
...
int main(int argc, char **argv) {
  ...
  string onnx_model_path("model_test_onnx_convT2D.onnx");
  string target_metric_file("metric_test_onnx_convT2D.txt");
  ...
  if (!only_import) {
    ...
    // Decoder
    l = ReLu(ConvT2D(l, 32, {2, 2}, {2, 2}));
    // x = [batch, 32, 12, 12]
    l = ReLu(PointwiseConv2D(l, 16));
    // x = [batch, 16, 12, 12]
    l = ReLu(ConvT2D(l, 16, {3, 3}, {1, 1}, "valid"));
    // x = [batch, 16, 14, 14]
    l = ReLu(ConvT2D(l, 16, {2, 2}, {2, 2}));
    // x = [batch, 16, 28, 28]
