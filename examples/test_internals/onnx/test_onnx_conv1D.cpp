...
///////////////////////////////////
// test_onnx_conv1D.cpp:
// A MNIST example with conv1D net
// using 28x28 images as sequences
// of 28 length with elements of 28
// elements. For ONNX testing
///////////////////////////////////

int main(int argc, char **argv) {
  bool use_cpu = false;
  bool only_import = false;
  string onnx_model_path("model_test_onnx_conv1D.onnx");
  string target_metric_file("metric_test_onnx_conv1D.txt");
  ...
  if (!only_import) {
    ...
    l = MaxPool1D(ReLu(Conv1D(l, 16, {3}, {1}, "valid")), {2}, {2});
    l = ReLu(Conv1D(l, 32, {5}, {2}, "valid"));
    l = MaxPool1D(ReLu(Conv1D(l, 32, {2}, {1}, "valid")), {3}, {3});
    l = AveragePool1D(ReLu(Conv1D(l, 64, {3}, {1}, "valid", false)), {4}, {4});
    l = Reshape(l, {-1});
