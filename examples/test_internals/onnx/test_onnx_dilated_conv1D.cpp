...
///////////////////////////////////////////////////////////////
// test_onnx_dilated_conv1D.cpp:
// A MNIST example with conv1D net (with dilated convolutions)
// using 28x28 images as sequences of 28 length with elements
// of 28 elements. For ONNX testing
///////////////////////////////////////////////////////////////

int main(int argc, char **argv) {
  bool use_cpu = false;
  bool only_import = false;
  string onnx_model_path("model_test_onnx_dilated_conv1D.onnx");
  ...
  if (!only_import) {
    ...
    l = MaxPool1D(ReLu(Conv1D(l, 16, {5}, {1}, "same", true, 1, {2})), {2}, {2});
    l = ReLu(Conv1D(l, 32, {3}, {1}, "valid", true, 1, {4}));
    l = MaxPool1D(ReLu(Conv1D(l, 32, {2}, {1}, "valid", false, 1, {3})), {3}, {3});
    l = Flatten(l);
