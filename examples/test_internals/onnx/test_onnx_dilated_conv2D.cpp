...
////////////////////////////////////////
// test_onnx_dilated_conv2D.cpp:
// A MNIST example with conv2D net
// (dilated conv2D) to test ONNX module
////////////////////////////////////////

int main(int argc, char **argv) {
  bool use_cpu = false;
  bool only_import = false;
  string onnx_model_path("model_test_onnx_dilated_conv2D.onnx");
  ...
  if (!only_import) {
    ...
    l = Reshape(l, {1, 28, 28});
    l = MaxPool2D(ReLu(Conv2D(l, 16, {5, 5}, {1, 1}, "same", true, 1, {2, 2})), {2, 2});
    l = ReLu(Conv2D(l, 16, {3, 3}, {1, 1}, "same", true, 1, {3, 3}));
    l = GlobalAveragePool2D(ReLu(Conv2D(l, 16, {2, 2}, {1, 1}, "valid", false, 1, {4, 4})));
    l = Flatten(l);
