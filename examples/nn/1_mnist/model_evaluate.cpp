...
int main(int argc, char **argv) {
    ...
    ofstream file;
    file.open("sal-cifar.txt", fstream::app);
  ...
  //net=Model({in},{out});
   ...
   //net->verbosity_level = 0;
    ...
    // Load weights
    if(testing){
        net = import_net_from_onnx_file("saved-cifar.onnx", dev);
	out = net->lout[0];
    }else{
	//ONNX
        net = import_net_from_onnx_file("model.onnx");
	out = net->lout[0];
	//BIN
	//load(net, "vgg16-full.bin");
    }
    ...
    Tensor *batch = Tensor::empty({batch_size, 256, 256, 3});
