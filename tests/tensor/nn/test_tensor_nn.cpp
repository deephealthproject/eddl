...
TEST(TensorTestSuite, tensor_nn_full_softmax){
    ...
    // Backward: CPU
    Tensor* t_cpu_delta = Tensor::zeros_like(t_cpu_in);
    Tensor* t_cpu_parent_delta = t_cpu_in;  // Whatever
    tensorNN::D_FullSoftmax(t_cpu_delta, t_cpu_out, t_cpu_parent_delta);

    // Backward: GPU
    Tensor* t_gpu_delta = Tensor::zeros_like(t_gpu_in);
    Tensor* t_gpu_parent_delta = t_gpu_in;  // Whatever
    ...
    delete t_cpu_delta;
    delete t_gpu_delta;
    // delete t_cpu_parent_delta;  // Aliases
    // delete t_gpu_parent_delta;  // Aliases
