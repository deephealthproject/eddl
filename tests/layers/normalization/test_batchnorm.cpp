...
TEST(NormalizationTestSuite, batchnorm){
    ...
    ASSERT_TRUE((bool) Tensor::equivalent(t_fwrd_ref, t_output, 1e-2f));
    ASSERT_TRUE((bool) Tensor::equivalent(t_mean_ref, t_mean, 1e-2f));
    ASSERT_TRUE((bool) Tensor::equivalent(t_var_ref, t_var, 1e-2f));
    int asd = 33;
}
