...
//    }
    ...
    // *** [CREATE::EYE] *********************************************
   t_input = Tensor::ones({10, 10});


   vector<string> idxs = {":5", ":5"};
   res_small_cpu = run_tensor_select(t_input, "select", idxs, DEV_CPU, 1);
   res_small_gpu = run_tensor_select(t_input, "select", idxs, DEV_GPU, 1);
